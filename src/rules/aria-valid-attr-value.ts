...
function valid(el: Element, attribute: string, info: Info) {
  ...
  if (info.type === 'boolean' && (value === 'true' || value === 'false')) {
    ...
    if (el.id === 'pass169') console.log(el.id, value, referencedValue, document.querySelectorAll('div'))
    ...
    //if (referencedValue.textContent?.trim() === '') return false
    return true 
  } else if (info.type === 'nmtoken' && (info.values?.includes(value) || (info.allowEmpty && value.trim() === ''))) {
    if (info.allowEmpty && value === '') return true
    ...
    return true 
  } else if (info.type === 'string' && (value.trim() !== '' || info.allowEmpty)) {
    return true 
  }
  return false
}
...
export function ariaValidAttrValue(el: Element): AccessibilityError[] {
  const errors = []
  const selector = Object.keys(ariaAttributes).map(attributeName => `[${attributeName}]`).join(',')
